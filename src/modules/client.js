...
let myUsername = 'wony';
let opaqueId = 'test'; // 무슨 역할?

if (window.location.protocol === 'http:') server = 'http://' + 'localhost:8088' + '/janus';
else server = 'https://' + 'localhost:8088' + '/janus';

export function initJanus() {
	Janus.init({
		debug: 'all',
		callback: function () {
			janus = new Janus({
...
function localScreenFeed(msg) {
	janus.attach({
		...
		success: function (pluginHandle) {
			...
			Janus.log(
				'Plugin attached! (' + screenHandle.getPlugin() + ', id=' + screenHandle.getId() + ')'
			);
...
function mobileFeed(id) {
	...
	janus.attach({
		...
		success: function (pluginHandle) {
			...
			Janus.log('Plugin attached! (' + mobileFeed.getPlugin() + ', id=' + mobileFeed.getId() + ')');
