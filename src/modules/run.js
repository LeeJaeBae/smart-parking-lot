...
if (window.location.protocol === 'http:') server = 'localhost:8088/janus';
else server = 'localhot:8088/janus';
				...
				success: function () {
					...
					janus.attach({
						...
						onmessage: function (msg, jsep) {
							Janus.debug(' --Janus-- ::: Got a message (publisher) :::', msg);
							...
							if (event) {
								if (event === 'joined') {
									...
									Janus.log(` --Janus-- Room : ${msg['room']}에 성공적으로 접속하였습니다.`);
									...
									Janus.log(' --Janus-- Got a list of available publishers/feeds:', list);
...
function localScreenFeed(msg) {
	janus.attach({
		...
		mediaState: function (medium, on) {
			Janus.log(
				' --Janus-- Screen Janus ' + (on ? 'started' : 'stopped') + ' receiving our ' + medium
			);
		},
		error: function (error) {
